...
- **Sleep / wake during pause auto-finalizes.** `sourceInterrupted` events from ScreenCaptureKit / mic stalls are not gated by `paused`, so a Mac going to sleep while the user has paused the recording will route through the existing capture-failure path and end the meeting. Worst-case behavior is a clean save of whatever was captured before sleep — no data loss — but the user expecting to resume after wake gets a finalized meeting instead. A follow-up PR can either auto-pause-stop on `NSWorkspace.willSleepNotification` or attempt a stream re-init on wake.
