...
Mic Input    → AVAudioEngine (raw tap) → Input Node Tap ─────────────┘   (AsyncStream<MeetingAudioCaptureEvent>)
                                   ...
                                   - VPIOConditioner (explicit opt-in only)
                                              ...
                                              → convert to 16kHz mono WAV → Parakeet STT
```
...
- Final meeting STT still runs on a converted `16kHz` mono WAV, not directly on the stereo artifact. The live transcript may contribute source/speaker structure through `preparedTranscript`. See `docs/research/meeting-dual-stream-transcription-pipeline.md` for the full pipeline and tradeoffs.
