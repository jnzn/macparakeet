...
/// and a pulsing center. Sacred geometry vibes in a 26×26 frame.
private struct SpinnerRingView: View {
    ...
    var body: some View {
        ZStack {
            // Outer guide ring (very faint, grounds the geometry)
            Circle()
                .stroke(Color.white.opacity(0.05), lineWidth: 0.5)
                .frame(width: size, height: size)

            // Triangle 1 — clockwise, brighter
            triangleLayer(rotation: rotationCW, opacity: 0.7, vertexOpacity: vertexPulse)

            // Triangle 2 — counter-clockwise, softer
            triangleLayer(rotation: rotationCCW, opacity: 0.4, vertexOpacity: vertexPulse * 0.7)

            // Center nexus — pulsing core
            ZStack {
                Circle()
                    .fill(Color.white.opacity(centerPulse * 0.5))
                    .frame(width: 10, height: 10)
                    .blur(radius: 4)

                Circle()
                    .fill(Color.white.opacity(centerPulse))
    ...
    private func triangleLayer(rotation: Double, opacity: Double, vertexOpacity: Double) -> some View {
        ZStack {
            // Triangle edges
            TriangleShape()
                .stroke(Color.white.opacity(opacity * 0.5), lineWidth: 0.8)
    ...
    private func vertexDot(index: Int, vertexOpacity: Double) -> some View {
        ...
        return ZStack {
            Circle()
                .fill(Color.white.opacity(vertexOpacity * 0.3))
                .frame(width: 7, height: 7)
                .blur(radius: 3)

            Circle()
                .fill(Color.white.opacity(vertexOpacity))
...
private struct TriangleShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for i in 0..<3 {
            ...
            let point = CGPoint(x: center.x + Foundation.cos(angle) * radius, y: center.y + Foundation.sin(angle) * radius)
