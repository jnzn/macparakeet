...
///
/// A bare unit digit (zero–nine) standing alone in prose is left as a word —
/// "the one thing", "that one", "the answer is zero", "call me at four" all
/// read better spelled out, and "one" in particular doubles as an indefinite
/// pronoun/determiner in everyday English. It only converts when adjacent to
/// another spoken number word, i.e. part of a compound or a digit sequence.
/// Teens/tens standing alone ("twenty") don't carry this ambiguity and always
/// convert. See `isAdjacentToNumberWord`.
public enum NumberNormalizer {
    ...
    private static func parseNumberRun(_ tokens: [Token], from start: Int) -> (String, Int)? {
        ...
        // A lone unit digit (zero–nine) reads as a word far more often than as
        // a numeral when it stands alone in prose — "the one thing", "that
        // one", "just one", "the answer is zero", "call me at four" all read
        // better spelled out than "the 1 thing" / "the answer is 0". Only
    ...
    private static func previousWordIndex(_ tokens: [Token], before i: Int) -> Int? {
        var j = i - 1
        while j >= 0 {
            if case .word = tokens[j] { return j }
            j -= 1
    ...
    private static func previousWord(_ tokens: [Token], before i: Int) -> String? {
        guard let j = previousWordIndex(tokens, before: i), case .word(let w) = tokens[j] else { return nil }
        return w.lowercased()
    }

    /// True when the number word at `index` sits immediately next to another
    /// spoken number word (only punctuation/whitespace between them) — i.e.
    /// it reads as part of a digit sequence ("one two three") rather than a
