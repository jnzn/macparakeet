...
public actor STTRuntime: STTRuntimeProtocol {
    ...
    /// Intended to keep the interactive slot's model hot between dictations.
    /// The FluidAudio API no longer accepts raw float buffers directly, so
    /// this is a no-op — the ANE manages its own model residency.
