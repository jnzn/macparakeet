...
| P1 | 2 |
| P2 | 16 |
| Finding status | 7 OPEN · 9 FIXED (PDX-001/002/007/012/013/014/016/018, +006 pending runtime check) · 1 ACCEPTED (PDX-009) · 1 DECLINED (PDX-005) |
...
| PDX-005 | No runtime integrity check of the dylib/model; SHA gate never armed in production | P2 | DECLINED | Pass-1 P1, tempered. The SHA256 check runs only when `configuration.modelSHA256 != nil` (`MeetingEchoSuppressionRuntime.swift:216-221`), and that value comes *only* from `MACPARAKEET_MEETING_ECHO_MODEL_SHA256` (`:55-59`) — a build-time-only var the production `.fromEnvironment()` path never sets. So at runtime the dylib is never hashed and the model is verified only `fileExists`/`isReadable` before `dlopen`. A tampered on-disk `Contents/Frameworks/liblocalvqe.dylib` or `.gguf` loads silently (the self-signed PDX bundle has `flags=none`, so the OS does not re-validate on launch). Tempered to P2: needs local bundle-write. Direction: pin and verify a SHA for both the dylib and model at load in release builds; the passthrough fallback path already exists (`:219`). **Update (2026-06-04, DECLINED):** implemented (runtime verify + a post-sign Info.plist hash injection) then reverted at owner request — the build-signing complexity wasn't worth it for a P2 gated on local bundle-write. Learning kept: the *model* and the *post-sign dylib* hash CAN both be pinned (the "dylib can't be pinned" framing was wrong — compute it after `codesign`; confirmed seed `9e13…` vs signed `5c2c…` differ). PDX-006's sample-rate check stays. To re-arm: pin both hashes into Info.plist during signing (after the final `--deep`, then re-seal shallow so the dylib isn't touched again) and verify them in `makeConditioner` against `MPKMeetingEchoModelSHA256`/`MPKMeetingEchoLibrarySHA256`. |
| PDX-006 | `localvqe_sample_rate` read into a dead property, never validated against 16 kHz capture | P2 | FIXED | `MeetingEchoSuppressionRuntime.swift:322-325`/`:350` read the lib's reported sample rate into `self.sampleRate`, but `processor.sampleRate` is consumed nowhere. Audio is hard-resampled to 16 kHz upstream (`AudioChunker`), and only frame/hop is reconciled at runtime (`:351`). If the bundled lib were ever built for another rate, 16 kHz frames would be pushed through a mismatched model; the only output check is `count == frameSize`, which still passes, so it would emit corrupted audio into the STT-bound copy. The on-disk recording keeps raw mic, so impact is degraded transcription, not lost audio. Direction: compare reported rate to capture rate at load; fall back to passthrough on mismatch. **Update (2026-06-04, FIXED — pending runtime check):** `makeDynamicConditioner` now compares the library's reported rate to the configured 16 kHz and falls back to passthrough on mismatch, logging the reported rate on every load (`meeting_echo_loaded reported_sample_rate=…` / `meeting_echo_sample_rate_mismatch …`; subsystem `com.macparakeet.core`, category `MeetingEchoSuppression`). Not headlessly testable (needs the dylib); owner accepted the silent-fallback risk and will confirm against live-meeting logs. App builds; 29 echo tests green. |
| PDX-007 | `cleanupFailedStart` cancels but does not drain the processing task before freeing the conditioner | P2 | FIXED | `MeetingRecordingService.swift:459-466` cancels `processingTask` and runs `cleanupState()` (which drops the last conditioner ref → `deinit` → `freeFunction(context)` + `dlclose`) **without** `await task.value`. The stop/cancel paths correctly `await drainProcessingTaskAfterCaptureStop()` first; `cleanupFailedStart` is the one cleanup path that skips the drain. Latent today (no throwing `try await` exists after the task is created at `:439`, so the task is effectively nil here), but adding one would make a free-while-C-call-in-flight race reachable. Direction: route `cleanupFailedStart` through the same drain. **Update (2026-06-05, FIXED):** `cleanupFailedStart` now `await audioCaptureService.stop()` (idempotent — finishes the events stream) + `await drainProcessingTaskAfterCaptureStop()` before `cleanupState()`, so the processing + ingest tasks are drained (or cancelled-and-awaited) before the conditioner is freed — mirroring the stop/cancel paths and folding in the former manual ingest-queue teardown. Defensive (still latent), so no new unit test, but the failed-start + drain tests (incl. `testStartRecordingCleansUpWhenLockWriteFails`) stay green. |
...
| PDX-009 | `unsafeBitCast` of every `dlsym` result to a `@convention(c)` pointer with no ABI guard | P2 | ACCEPTED | `MeetingEchoSuppressionRuntime.swift:408-426` bit-casts each resolved symbol to its expected function type. A *missing* symbol is handled (throws → passthrough), but a *present* symbol with a different real signature (tampered or version-skewed lib) is invoked with the wrong ABI — undefined behavior. Inherent to `dlsym`; pairs with PDX-005. Direction: pin the dylib by hash (PDX-005) and/or check a `localvqe_abi_version` symbol before first use. **Update (2026-06-04, ACCEPTED):** owner accepts the limitation. A robust ABI handshake needs the closed-source binary to export a version symbol (it doesn't), and the wrong-ABI call only triggers if the dylib has already been swapped in the bundle — which requires local code-execution. Real mitigation, if ever wanted, is dylib integrity via signing (PDX-005 dylib half / PDX-004), not an in-Swift guard. |
...
| PDX-012 | `llm_runs` ledger grows unbounded — one row per LLM call, forever | P2 | FIXED | `LLMRunRepository.save` is an unconditional insert with a fresh UUID per run (`LLMRunRepository.swift:29-33`, `LLMRun.swift:67`), recorded on every dictation and transcription; the repo exposes only `count()`/`deleteAll()` — no prune, cap, or age-out. A heavy user accrues a row per LLM invocation indefinitely. Not a privacy leak (metadata only — provider/model names, token/char counts, latency, `errorType`; no prompt/transcript/output) and degrades gracefully (recorder swallows save failures), but the table is unbounded. Direction: bounded-retention sweep (cap N rows or age out) on insert or launch. **Update (2026-06-04, FIXED):** `LLMRunRepository.save` now enforces a row-count retention cap inside the write transaction (`enforceRetentionLimit`, default 5000, injectable). Chose a count cap over age-out because the finding is unbounded *footprint* — a count cap bounds it directly regardless of usage rate (an age cap wouldn't bound a burst). Metadata-only rows so the cap is ~1 MB. Unit-tested (newest-N kept, oldest pruned); existing repo tests green. |
| PDX-013 | Dormant remote free-text submitter wired to a hardcoded domain | P2 | FIXED | `DiscoverThoughtsService.submitThought` (`:28-47`) POSTs a free-text `message` plus a system-info fingerprint (`app_version`, `build_number`, `mac_os_version`, `chip_type`) to a hardcoded `https://macparakeet.com/api/discover-thoughts` (`:23`). Grep finds **no caller** — nothing user-dictated reaches it today. Flagged because once wired to a UI field the `message` is unscrubbed/unbounded and the system-info is finer-grained than telemetry's coarse buckets (AUDIT-022 class). Direction: keep it user-initiated-only; document that `message` is hand-typed feedback (never transcript); route system-info through telemetry's coarse buckets. **Update (2026-06-05, FIXED — deleted):** owner chose cleanup over hardening — the whole `DiscoverThoughtsService` (protocol + class + error enum, ~78 lines) was dead (zero references in Sources/Tests), so it's removed entirely. No latent phone-home left to wire up; if the feature is ever wanted it should be rebuilt user-initiated with bucketed system-info. App builds. |
...
| PDX-016 | `RhodoneaScribeLoader` `TimelineView` body allocates ~280 `Path`s + ~320 trig calls/frame | P2 | FIXED | `RhodoneaScribeLoader.swift:13-73`: the `.animation(minimumInterval: 1/60)` body builds a fresh 240-segment base `Path` plus 42 trail `Path`s per tick, each via `sin`/`cos`/`pow` (~17k `Path`s/sec). **Not** the most-important `TimelineView` the brief feared — its only consumer is the transient transform-progress pill (`TransformSpikeProgressPanelController.swift:284`), shown only while `.working` and `paused:` is wired to `reduceMotion`. Cost is real but lasts only the seconds a transform is in flight. Direction: precompute the size-invariant base curve once; cache trail geometry or drop to ~30 fps. **Update (2026-06-04, FIXED):** the 240-sample base curve is now precomputed once in unit space (`baseUnitPoints`) and only scaled per frame, eliminating ~240 of the ~320 per-frame sin/cos evaluations (the animated trail's 84 stay, since their phase tracks the clock). Behaviour-preserving refactor guarded by an equivalence test (precomputed curve == original per-point formula). |
...
| PDX-018 | `TransformsCoordinator` fires the `.failed` panel twice on one run | P2 | FIXED | The `onProgress` closure spawns a MainActor Task to call `panelController?.fail(...)` on any `.failed` event (`TransformsCoordinator.swift:227-236`), and the surrounding `catch` blocks *also* call `fail(...)` (`:298-326`). For a failed run both fire (both `activeRunID`-guarded, so cross-run is safe) — redundant work + possible pill flicker. No data loss (the executor restores the clipboard on failure). Direction: let the terminal `catch` own the fail state; drop `.failed` from the streaming callback. **Update (2026-06-05, FIXED):** dropped the `.failed` branch from `onProgress`; the do/catch now solely owns terminal state. Verified safe — every `onProgress(.failed)` in `TransformExecutor` is immediately paired with a `throw` (`:134→135`, `:143→144`, `:178→179`, `:225→226`, …), so the catch always runs; this also corrects the cancelled / llm-not-configured cases, where the catch intentionally `close()`s or shows provider setup rather than a fail pill. Removal-only; app builds. (Coordinator isn't unit-testable; the safety is the executor pairing.) |
...
- **Native binary integrity.** PDX-005 was implemented (runtime verify + post-sign
  Info.plist hash injection) then **reverted / DECLINED** (owner) — it's doable (the
  "dylib can't be pinned" blocker was wrong; compute it post-sign), but the build-signing
  complexity wasn't worth it for this local-write-gated P2. Re-arm steps are in the
  finding. PDX-009 is ACCEPTED (owner) — a robust ABI handshake needs a version symbol the
  binary doesn't export, and the wrong-ABI path only triggers if the dylib is already
  swapped locally.
